...
mt19937 _generator;
uniform_real_distribution<> Uniform(-delta_translation, delta_translation);
uniform_real_distribution<> Rndm(0.0, 1.0);
...
double variationalWaveFunction_second(double x)
{
    double mu=0.79;
    double sigma=0.64;
    return exp(-(x-mu)*(x-mu)/(2.*sigma*sigma)) * (mu*mu-sigma*sigma+x*x-2.*mu*x)/(sigma*sigma*sigma*sigma) +
...
void translation()
{
	...
	totalTranslations++;
	double delta = Uniform(_generator);
	...
	if(Rndm(_generator)<acceptance_probability)
	{
		for(int i=0;i<timeslices;i++)
			positions[i]+=delta;
...
void brownianBridge()
{
	...
	int starting_point = (int)(Rndm(_generator)*available_starting_points);
	...
	for(int i=0;i<brownianBridgeReconstructions;i++)
	{
        ...
        normal_distribution<> Gaus(average_position,sqrt(variance));
		double newcoordinate = Gaus(_generator);
...
void brownianMotion(int which) // BM is called only for PIGS simulations
{
	int starting_point, endpoint, left_reco;
        ...
        normal_distribution<> Gaus(average_position,sqrt(variance));
		starting_coord = Gaus(_generator);
