...
void Input(void){
    ...
    cout << "Number of equilibration steps = " << neq <<endl << endl;
    ReadInput.close();
...
void Averages(int iblk){ //Print results for current block
    ...
    for (int i=igofr; i<nbins+igofr; i++){
        ...
        Gofr << setw(wd) << r + bin_size/2.0 << setw(wd) << gdir << setw(wd) << endl;
        Gave << setw(wd) << r + bin_size/2.0 << setw(wd) << glob_av[i]/(double)iblk << setw(wd) << Error(glob_av[i],glob_av2[i],iblk) << endl;
